...
template <class HashedObj>
int QuadraticHashTable<HashedObj>::findPosInt(int x ) const
{
/* 1*/      int collisionNum = 0;
/* 2*/      int currentPos = hash( x, array.size( ) );
///
...
template <class HashedObj>
const HashedObj & QuadraticHashTable<HashedObj>::findInt( int x ) const
{
    int currentPos = findPosInt( x );
    return isActive(currentPos) ? array[currentPos].element : ITEM_NOT_FOUND;
}
...
template<class HashedObj>
int QuadraticHashTable<HashedObj>::hash(HashBlock key, int tableSize) const
{
    int key2 = key.oldLoc;
    if(key2 < 0) key2 = -key2;
